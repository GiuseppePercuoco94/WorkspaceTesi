...
def investigate_requests(token,list_d):
    ...
    if os.path.exists(folder_base+'/score_scanned'):
        print('folder scanned exists')
    else:
        os.mkdir(folder_base+'/score_scanned')
        
    if os.path.exists(folder_base+'/score_scanned/'+list_dn.split('/')[-1]):
        os.remove(folder_base+'/score_scanned/'+list_dn.split('/')[-1])
        print('old '+ list_dn.split('/')[-1]+ ' removed')
    else:
        print('no old '+ list_dn.split('/')[-1]+' exists')
    
    if os.path.exists(folder_base+'/score_error_umb'):
        print('folder error_umb exists')
    else:
        os.mkdir(folder_base+'/score_error_umb')
    
    if os.path.exists(folder_base+'/score_error_umb/'+list_dn.split('/')[-1].split('.')[0]+'_error_umb.txt'):
        print('relative error file exists..removing it')
        os.remove(folder_base+'/score_error_umb/'+list_dn.split('/')[-1].split('.')[0]+'_error_umb.txt')
    else:
    ...
    '''
    header_vt = ['domain','INV_score']
    #header  = ['domain','status','security_categories','content_categories']
    data_to_write = []
    count_dom = 0
    
    scanned_folder_path = os.path.join(folder_base,'score_scanned')
    file_name_out = os.path.join(scanned_folder_path,list_dn.split('/')[-1])
    scanned_folder_path = os.path.join(folder_base,'score_error_umb')
    ...
    with open(file_name_out, mode='a') as csv_out, open(error_file_name_out, mode='w+') as txt_deb:
        ...
        for domain in domains:
            count_dom += 1
            response = requests.get('https://investigate.api.umbrella.com/domains/risk-score/' + domain , params=params, headers=header_api)
            ...
            data_to_write.append(str(response_json['risk_score']))
            #print(count_dom)
